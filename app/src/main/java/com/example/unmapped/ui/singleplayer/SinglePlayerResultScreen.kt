// IN: ui/singleplayer/SinglePlayerResultScreen.kt
...
import androidx.compose.ui.Alignment // KORREKTUR: Fehlender Import
...
/**
 * Stellt den Ergebnisbildschirm nach einer einzelnen Runde im Einzelspielermodus dar.
 */
@Composable
fun SinglePlayerResultScreen(
    navController: NavHostController,
    viewModel: GameViewModel
) {
    val gameState by viewModel.gameState.collectAsState()
    // Sicherheitsprüfung, um sicherzustellen, dass gültige Ergebnisdaten vorhanden sind.
    if (gameState.gameRounds.isEmpty() || gameState.gameRounds[gameState.currentRoundIndex].result == null) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        ResultDetailsCard(modifier = Modifier.align(Alignment.BottomCenter)) {
            // ... (Detail-UI mit Distanz, Score und Button)
            Text(
            ...
            Button(
                onClick = {
                    if (isLastRound) {
                        viewModel.nextRound()
                        navController.navigate(Screen.GameEnd.route) { popUpTo(Screen.GamePlay.route) { inclusive = true } }
                    } else {
                        viewModel.nextRound()
                ...
                Text(if (isLastRound) "End the misery." else "Go on then.", fontSize = 18.sp, fontWeight = FontWeight.Bold)
            }
        }
    }
}